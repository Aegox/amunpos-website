// Fuente: https://alignui.com/docs/v1.2/ui/modal (código real, verbatim)
...
>(({ className, ...rest }, forwardedRef) => {
  return <DialogPrimitive.Title ref={forwardedRef} className={cn('text-label-sm text-text-strong-950', className)} {...rest} />;
});
...
>(({ className, ...rest }, forwardedRef) => {
  return <DialogPrimitive.Description ref={forwardedRef} className={cn('text-paragraph-xs text-text-sub-600', className)} {...rest} />;
});
...
function ModalFooter({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('flex items-center justify-between gap-3 border-t border-stroke-soft-200 px-5 py-4', className)} {...rest} />;
}
...
/*
  ...
  `src/components/ui/modal.tsx` reutiliza exactamente esta estructura
  (Root/Trigger/Content/Header/Body/Footer sobre Radix Dialog) para
  reemplazar el modal de auth hecho a mano (div fijo + botón de cerrar rojo)
  que el usuario señaló como "dejado atrás". El botón de cerrar usa nuestro
  Button variant="ghost" size="xsmall" en vez de un CompactButton dedicado
  (no existía ese componente en el sitio y no vale la pena crear uno nuevo
  solo para un botón de icono).
*/
