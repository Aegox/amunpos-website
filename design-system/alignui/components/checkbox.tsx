// Fuente: https://alignui.com/docs/v1.2/ui/checkbox (código real, verbatim)
...
>(({ className, checked, ...rest }, forwardedRef) => {
  ...
  // precalculado con .getTotalLength()
    ...
    >
      ...
      <CheckboxPrimitive.Indicator forceMount className='[&_path]:transition-all [&_path]:duration-300 [&_path]:ease-out [&_svg]:opacity-0'>
        <IconCheck
          ...
          style={{ ['--total-length' as any]: TOTAL_LENGTH_CHECK }}
        />
        <IconIndeterminate
          className={cn(
            'absolute left-1/2 top-1/2 shrink-0 -translate-x-1/2 -translate-y-1/2',
          ...
          style={{ ['--total-length' as any]: TOTAL_LENGTH_INDETERMINATE }}
        />
...
/*
  Detalle notable: el check y el indeterminate se "dibujan" con una animación
  de stroke-dasharray/stroke-dashoffset (como si se trazaran a mano), no con
  un simple fade/opacity — por eso las longitudes están precalculadas
  (getTotalLength()). Ver icons/checkbox-check.svg e icons/checkbox-indeterminate.svg
  en esta misma carpeta para los paths exactos ya extraídos.
*/
