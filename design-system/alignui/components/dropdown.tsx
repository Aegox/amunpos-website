// Fuente: https://alignui.com/docs/v1.2/ui/dropdown (código real, verbatim)
...
function DropdownItemIcon<T extends React.ElementType>({ className, as, ...rest }: PolymorphicComponentProps<T>) {
  ...
  return <Component className={cn('size-5 text-text-sub-600', 'group-has-[[data-disabled]]:text-text-disabled-300', className)} {...rest} />;
}
...
>(({ className, ...rest }, forwardedRef) => <DropdownMenuPrimitive.Group ref={forwardedRef} className={cn('flex flex-col gap-1', className)} {...rest} />);
...
>(({ className, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.Label ref={forwardedRef} className={cn('px-2 py-1 text-subheading-xs uppercase text-text-soft-400', className)} {...rest} />
));
