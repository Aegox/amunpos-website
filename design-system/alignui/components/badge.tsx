// Fuente: https://alignui.com/docs/v1.2/ui/badge (código real, verbatim)
...
export const badgeVariants = tv({
  ...
  variants: {
    size: {
      small: { root: 'h-4 gap-1.5 px-2 text-subheading-2xs uppercase has-[>.dot]:gap-2', icon: '-mx-1 size-3', dot: '-mx-2 size-4' },
      medium: { root: 'h-5 gap-1.5 px-2 text-label-xs', icon: '-mx-1 size-4', dot: '-mx-1.5 size-4' },
    },
    variant: {
    ...
    // 10 familias semánticas reales — nótese "purple" (color de UI, elegible por
    // el consumidor) mapea al token semántico "feature" (el nombre que usa
    // AlignUI internamente para "esto es una capacidad especial/destacada",
    // p. ej. IA). No es casualidad: es literalmente su convención para
    // resaltar funciones avanzadas.
    color: { gray: {}, blue: {}, orange: {}, red: {}, green: {}, yellow: {}, purple: {}, sky: {}, pink: {}, teal: {} },
    disabled: { true: { root: 'pointer-events-none' } },
    square: { true: {} },
  },
  compoundVariants: [
    // variant=filled
    ...
    // variant=lighter
    { variant: 'lighter', color: 'gray', class: { root: 'bg-faded-lighter text-faded-base' } },
    { variant: 'lighter', color: 'blue', class: { root: 'bg-information-lighter text-information-base' } },
    { variant: 'lighter', color: 'orange', class: { root: 'bg-warning-lighter text-warning-base' } },
    { variant: 'lighter', color: 'red', class: { root: 'bg-error-lighter text-error-base' } },
    { variant: 'lighter', color: 'green', class: { root: 'bg-success-lighter text-success-base' } },
    { variant: 'lighter', color: 'yellow', class: { root: 'bg-away-lighter text-away-base' } },
    { variant: 'lighter', color: 'purple', class: { root: 'bg-feature-lighter text-feature-base' } },
    { variant: 'lighter', color: 'sky', class: { root: 'bg-verified-lighter text-verified-base' } },
    { variant: 'lighter', color: 'pink', class: { root: 'bg-highlighted-lighter text-highlighted-base' } },
    { variant: 'lighter', color: 'teal', class: { root: 'bg-stable-lighter text-stable-base' } },
    // variant=stroke
    ...
    // disabled
    {
      ...
      class: { root: ['ring-1 ring-inset ring-stroke-soft-200', 'bg-transparent text-text-disabled-300'] },
    },
  ],
...
type BadgeRootProps = VariantProps<typeof badgeVariants> & React.HTMLAttributes<HTMLDivElement> & { asChild?: boolean };
...
const BadgeRoot = React.forwardRef<HTMLDivElement, BadgeRootProps>(
  ({ asChild, size, variant, color, disabled, square, children, className, ...rest }, forwardedRef) => {
    ...
    const extendedChildren = recursiveCloneChildren(children as React.ReactElement[], sharedProps, [BADGE_ICON_NAME, BADGE_DOT_NAME], uniqueId, asChild);
...
function BadgeIcon<T extends React.ElementType>({ className, size, variant, color, as, ...rest }: PolymorphicComponentProps<T, BadgeSharedProps>) {
