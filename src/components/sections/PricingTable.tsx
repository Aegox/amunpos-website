...
const caracteristicas: { icono: React.ComponentType<{ className?: string }>; nombre: string }[] = [
...
const planes: {
  ...
  },
  {
    ...
    mes: 79,
    valores: ["Ilimitadas", "Ilimitados", true, true, true, true, true, "Prioritario"],
  },
];

/** Un 20 % menos pagando por año, redondeado a entero. */
const DESCUENTO_ANUAL = 0.2;
const precioDe = (mes: number, anual: boolean) => (anual ? Math.round(mes * (1 - DESCUENTO_ANUAL)) : mes);
...
function Marca({ valor, oscuro }: { valor: Valor; oscuro?: boolean }) {
  ...
  if (valor === false)
    return <RiSubtractLine className={cn("size-[18px] shrink-0", oscuro ? "text-gray-700" : "text-gray-300")} />;
  if (valor === true)
    return <RiCheckLine className={cn("size-[18px] shrink-0", oscuro ? "text-ai-base" : "text-primary-base")} />;
  return <span className={cn("text-paragraph-sm", oscuro ? "text-gray-0" : "text-gray-900")}>{valor}</span>;
}

function Cabecera({ plan, anual, oscuro }: { plan: (typeof planes)[number]; anual: boolean; oscuro?: boolean }) {
  const Icono = plan.icono;
  return (
    <>
      <div className="flex items-center justify-between">
        <Icono className={cn("size-5", oscuro ? "text-ai-base" : "text-gray-400")} />
        {plan.destacado && (
      ...
      <div className={cn("mt-4 text-label-lg", oscuro ? "text-gray-0" : "text-gray-900")}>{plan.nombre}</div>
      <p className={cn("mt-1 text-paragraph-sm", oscuro ? "text-gray-450" : "text-gray-600")}>{plan.para}</p>

      <div className="mt-5 flex items-end gap-2">
        {/* `tabular-nums` para que al pasar de 49 a 39 no baile el ancho. */}
        <span className={cn("text-title-h4 tabular-nums", oscuro ? "text-gray-0" : "text-gray-900")}>
          ${precioDe(plan.mes, anual)}
        </span>
        <span className={cn("pb-1.5 text-paragraph-xs leading-4", oscuro ? "text-gray-450" : "text-gray-600")}>
          al mes
          <br />
          por sucursal
        </span>
...
export default function PricingTable() {
  ...
  return (
    <div className="w-full">
      ...
      {/* ── La tarjeta única ── */}
      <div className="relative rounded-[24px] bg-gray-0 p-2 ring-1 ring-gray-200 xl:p-3">
        {/* Escritorio: una rejilla donde cada fila es un concepto */}
        <div data-tabla className="hidden xl:grid xl:grid-cols-[minmax(0,240px)_repeat(3,minmax(0,1fr))]">
          ...
          <div className="p-5">
            <div className="text-label-sm text-gray-900">Todos los planes incluyen</div>
            <p className="mt-1 text-paragraph-sm text-gray-600">
              La app completa y las actualizaciones. Sin permanencia: te vas cuando quieras.
            </p>
          </div>
          {planes.map((plan) => (
            <div
              key={plan.id}
              className={cn("flex flex-col p-5", plan.destacado && "rounded-t-[18px] bg-gray-950 px-6")}
            >
          ...
          {caracteristicas.map((c, fila) => (
            <React.Fragment key={c.nombre}>
              <div className="flex items-center gap-2.5 border-t border-gray-100 px-5 py-3.5">
                ...
                <span className="text-paragraph-sm text-gray-700">{c.nombre}</span>
              </div>
              {planes.map((plan) => (
                <div
                  key={plan.id}
                  className={cn(
                    ...
                    // pila de celdas.
                    plan.destacado && fila === caracteristicas.length - 1 && "rounded-b-[18px]",
                  )}
                >
        ...
        <div data-tarjetas className="flex flex-col gap-3 xl:hidden">
          {planes.map((plan) => (
            <div
              key={plan.id}
              className={cn("flex flex-col rounded-[18px] p-5", plan.destacado ? "bg-gray-950" : "bg-gray-25")}
            >
              <Cabecera plan={plan} anual={anual} oscuro={plan.destacado} />
                  ...
                  <li key={c.nombre} className="flex items-center gap-2.5">
                    ...
                    <span
                      className={cn(
                        ...
                        plan.valores[i] === false && "line-through opacity-60",
                      )}
                    >
                      {c.nombre}
                    </span>
